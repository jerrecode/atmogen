...
def mixture_molar_heat_capacity_j_mol_k(
    ...
    """Return ideal-mixture molar constant-pressure heat capacity for FAST/STANDARD use."""
    ...
    cp = sum((value / total) * database.get(key).heat_capacity_j_mol_k for key, value in positive.items())
...
def dry_adiabatic_log_pressure_gradient(
    ...
    return float(R_GAS / mixture_molar_heat_capacity_j_mol_k(mole_fractions, database))
...
def gray_radiative_temperature_profile(
    *,
    ...
    optical_depth_pressure_exponent: float = 1.0,
) -> np.ndarray:
    """Gray Eddington-shape profile normalized to the solved surface temperature.

    τ(P)=τ_s (P/P_s)^n and T^4 ∝ (τ+2/3).  Normalizing by the surface
    value keeps this routine a vertical-structure operator rather than a second,
    competing surface-energy solver.
    """
    pressure = np.asarray(pressure_pa, dtype=float)
    if pressure.ndim != 1 or pressure.size < 1 or np.any(~np.isfinite(pressure)) or np.any(pressure <= 0):
    ...
    if not np.isfinite(tau_s) or tau_s < 0:
        raise ValueError("longwave_optical_depth_surface must be finite and non-negative")
    if not np.isfinite(exponent) or exponent <= 0:
        raise ValueError("optical_depth_pressure_exponent must be finite and positive")
    tau = tau_s * np.power(np.clip(pressure / surface_pressure_pa, 0.0, 1.0), exponent)
    ratio = (tau + 2.0 / 3.0) / (tau_s + 2.0 / 3.0)
    return np.asarray(surface_temperature_k * np.power(ratio, 0.25), dtype=np.float64)
...
def dry_convective_adjustment(
    *,
    ...
    Pressure must run from high to low (surface toward top).  For every adjacent
    pair, the upper temperature is raised if the implied dlnT/dlnP exceeds the
    ideal dry-adiabatic gradient.  The returned mask identifies adjusted layers.
    """
    pressure = np.asarray(pressure_pa, dtype=float)
    ...
    if np.any(~np.isfinite(pressure)) or np.any(pressure <= 0) or np.any(np.diff(pressure) >= 0):
    ...
    for upper in range(1, adjusted.size):
        minimum_stable_upper = adjusted[upper - 1] * (pressure[upper] / pressure[upper - 1]) ** nabla_ad
